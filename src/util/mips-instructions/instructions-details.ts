...
class InstructionDetails {
    private _instructionList: string[];
    private _instructionUsages: string[];
    private _instructionUsageExamples: string[];
    private _instructionInfos: string[];
    private _instructionCtranslations: string[];
    constructor(instructionList: string[], instructionUsages: string[], instructionUsageExamples: string[], instructionInfos: string[], instructionCtranslations: string[]) {
        this._instructionList = instructionList;
        this._instructionUsages = instructionUsages;
        this._instructionUsageExamples = instructionUsageExamples;
        this._instructionInfos = instructionInfos;
        this._instructionCtranslations = instructionCtranslations;
    }
    ...
    getDetails(): [string, string, string, string, string][] {
        return this._instructionList.map(
            (instr, i) => [
                ...
                this._instructionCtranslations[i]
            ]
        );
    }
}

const RFormatInstructionDetails: InstructionDetails = new InstructionDetails(
    RFormatInstructionList.getAllInstructions(), [
        "add $rd, $rs, $rt",
        "addu $rd, $rs, $rt",
        "and $rd, $rs, $rt",
        "nor $rd, $rs, $rt",
...
const IFormatInstructionDetails: InstructionDetails = new InstructionDetails(
    IFormatInstructionList.getAllInstructions(), [
        ...
        "rt = rs + immediate",
        "rt = rs & immediate",
        "if (rs == rt) PC = PC + 4 + (immediate << 2)",
        "if (rs != rt) PC = PC + 4 + (immediate << 2)",
        "rt = MEM[rs + immediate] (byte)",
        "rt = MEM[rs + immediate] (halfword)",
        "rt = MEM[rs + immediate] (word); processor.LLbit = 1",
        "rt = immediate << 16",
        "rt = MEM[rs + immediate] (word)",
        "rt = rs | immediate",
        "rt = (rs < immediate) ? 1 : 0",
        "rt = (rs < immediate) ? 1 : 0",
        "MEM[rs + immediate] = rt (byte)",
        "if (processor.LLbit == 1) { MEM[rs + immediate] = rt; processor.LLbit = 0; }",
        "MEM[rs + immediate] = rt (halfword)",
        "MEM[rs + immediate] = rt (word)"
    ]
);
